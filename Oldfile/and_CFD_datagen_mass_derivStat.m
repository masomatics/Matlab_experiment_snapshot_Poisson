...
function [tilde_pys, derivative] = and_CFD_datagen_mass_derivStat...
   ...
   % deriv_loglike2 = zeros(5,N);
    ...
    for(m = 2 : (num_timepts+1))
        ...
        rxn_rate(1,:) = theta(1) * ones(N,1); 
        rxn_rate(2,:) = theta(2) * datmat(1,:,m-1);
        rxn_rate(3,:) = theta(3) * datmat(1,:,m-1);
        rxn_rate(4,:) = theta(4) * datmat(2,:,m-1).*datmat(1,:,m-1);
        rxn_rate(5,:) = theta(5) * datmat(2,:,m-1);
        ...
        deriv_loglike(4,:) = deriv_loglike(4,:) +  datmat(2,:,m-1).*datmat(1,:,m-1).*(rxn_mat(:,4)'*(datmat(:,:,m) - datmathat))/(sigV^2)  ; 
        deriv_loglike(5,:) = deriv_loglike(5,:) +  datmat(2,:,m-1).*(rxn_mat(:,5)'*(datmat(:,:,m) - datmathat)) /(sigV^2) ;
...
%[r,c,v] = ind2sub(size(temp2),find(abs(temp2 - temp1) > 80));
          ...
          %  tic,
            ...
            deriv_pym = permute(repmat(deriv_loglike, [1,1,num_particles]),[2,3,1]).* ...
                repmat(tilde_pym, [1,1,5]);
          %  toc
            

          %  tic,
            deriv_py = permute(mean(deriv_pym,1), [3,2,1]) ;   
            tilde_py = repmat(mean(tilde_pym,1),[5,1]);  
