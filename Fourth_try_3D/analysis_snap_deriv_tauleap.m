...
function [derivative, energy] = analysis_snap_deriv_tauleap(init, theta, tend, ...
   ...
   % include= [];
    ...
    observed = [1,2; 1,3; 2,3]; 
    [num_obs_inst, num_obs_dim] = size(observed);
    ...
    %Temporary variables    
    ycopy = zeros(num_obs_dim, num_particles, N); 
    xcopy = zeros(num_obs_dim, num_particles, N);    
    ...
    sigW_copy = zeros(num_obs_dim, num_particles, N);
...
%    debug_derivative = zeros(num_parameters, num_slices);
 ...
 %           includeon = 1;
            ...
            for(obs = 1: num_obs_inst)
             ycopy = repmat(snapshots(observed(obs,:),:,slice_index), [1,1,N]);
             xcopy = permute(repmat(dat_now(observed(obs,:),:),[1,1,num_particles])...
                ,[1,3,2]);
             %Each dimension shall have different observation noise. 
             sigW_copy = repmat(sigW(observed(obs,:)),[1, num_particles, N]);
             %distance_pair = min(squeeze(sum((xcopy - ycopy).^2.*(1./sigW_copy.^2),1)), 300);
             
             %%% NEW distance TAISAKU : Normalize Smartly
